...
if (isset($_POST['username'], $_POST['email'], $_POST['p'], $_POST['name'], $_POST['last'], $_POST['datenac'], $_POST['confirmpwd'], $_POST['password'])) {
   ...
   // check existing email  
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO usuarios_tb (nombres, apellidos, nacimiento, usuario, correo, contra, estado, tipo, salt, log) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            $insert_stmt->bind_param('ssssssssss',$name, $last, $date, $username, $email, $password, $estado, $tipo, $random_salt, $log);
