...
include_once '../assets/includes/db_conexion.php';
    ...
    if (isset($_POST['searcht']))
    {
        if (!empty($_POST['searcht']))
        {
            ...
            $stmt = $mysqli->query("SELECT nombres, correo, usuario FROM usuarios_tb WHERE tipo = 2 AND (nombres LIKE '%".$SearchString."%' OR usuario LIKE '%".$SearchString."%' OR apellidos LIKE '%".$SearchString."%' OR correo LIKE '%".$SearchString."%') LIMIT 6");
            $string = "";
            if ($stmt->num_rows > 0)
            {
